...
export default {
  name: 'submit_checkin',
  description:
    '帮学员提交布丁关卡打卡（v1 极简版：仅记录笔记 + 更新连续天数，不算积分 / 不发勋章 / 不算关卡解锁）。' +
    '当学员**明确**说"帮我打卡"/"提交打卡"/"今天的反思打到第 X 关"/"把今天的感想提交了"/"打这一关"/"完成打卡"/"submit checkin" 时使用。' +
    '⚠️ **严格前置流程**（违反任一步骤会被后端 HTTP 400 拒绝）：' +
    '\n  1. 必须先调 get_my_camps 拿到学员报名的训练营列表（确认营 + 学员当前所在 stage）' +
    '\n  2. 必须先调 get_stage(stageId) 拿到目标关卡的真实 tasks 数组（task.id + task.title + 难度档候选）' +
    '\n  3. 让学员**自己**勾选完成了哪些任务（**不要替学员选**）' +
    '\n  4. 让学员**自己**选难度档（basic / intermediate / full，必须明确说出来）' +
    '\n  5. 让学员**自己**写今日反思（reflection，必填）和疑问（question，必填；真无问题可写"暂无"但不能空字符串）' +
    '\n  6. 把以上信息组装好才调本工具' +
    '\n📝 **v1 限制提示**：本次提交只记录笔记 + 更新连续天数；积分 / 勋章 / 关卡解锁会在学员下次浏览器内打卡时一并刷新——展示打卡完成时**必须告诉学员这个限制**。' +
    '\n🔗 **跨工具编舞**：本工具是终点，前面 get_my_camps → get_stage 都是为它服务。提交成功后可顺手调 get_today_checkin 给学员看更新后的连续天数。' +
    '\n🚫 **禁止行为**：' +
    '\n  - 凭空捏造 task ID（必须从 get_stage 返回的 tasks[].id 中选）' +
    '\n  - 学员只是聊学习心得、倾诉情绪、问"今天该学什么"时**不要主动调**本工具——学员没明确说要提交' +
    '\n  - 替学员选难度 / 写反思 / 编造完成的任务（违背"学员真实学习决策"原则）' +
    '\n  - 一次提交失败后无限重试（撞 400 / 401 应按错误处理章节告诉学员，不要循环硬撞）',
